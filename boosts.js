...
Molpy.DefineBoosts = function() {
	...
	new Molpy.Boost({
		...
		desc: function(me) {
			if(!Molpy.Got('BananananaSblitz'))return Molpify(me.power, 1) + '% Sand for ' + MolpifyCountdown(me.countdown);
			else if(Molpy.Redacted.totalClicks < 1e7) return 'Beach digs will give Coal for ' + MolpifyCountdown(me.countdown);
		},
	...
	Molpy.EggCost = function() {
		...
		if(Molpy.Has('Bonemeal'),Infinity) return 
		return Infinity;
	}
	new Molpy.Boost({ 
		name: 'Dragon Queen',
	...
	new Molpy.Boost({
		...
		Add: function(amount) {
			this.AddSuper(amount);
			this.countdown += Math.max(Math.ceil((2000/(1+Molpy.Got('Incubator')) / Math.ceil(this.Level / 5))),100);
			Molpy.Boosts['DQ'].Refresh();
		},
	...
	new Molpy.Boost({ 
		...
		icon: 'diamond',
		group: 'stuff',
		stats: 'From defeating knights.',//' You have ' + Molpify(Molpy.Boosts['Diamond'].Level, 3) + ' Diamond' + plural(Molpy.Boosts['Diamond'].Level) + '.', 
		desc: function(me) {
	...
	new Molpy.Boost({
		...
		desc: function(){
			...
			if(Molpy.DragonLuck) str += 'Draconic Luck: ' + Molpify(Molpy.DragonLuck,3);
			...
			if(Molpy.Got('Cup of Tea') || Molpy.Got('Healing Potion') || Molpy.Got('Knights\' Blood') || Molpy.Got('Ethyl Alcohol') || 0) str+= '<br><u>Potions</u></div>';
			...
			if(Molpy.Got('Knight\'s Blood')) str += '<br>Knights\' Bloods: ' + Molpy.Level('Knight\'s Blood');// + Molpy.Has('Knight\'s Blood', 2+Molpy.Boosts['Knight\'s Blood'].power)?str += '<div class="rightjust">A+P</div>':str += '<div class="rightjust">A</div>';
...
Molpy.Annililate = function(){
	...
	if(!Molpy.Got('Chthonism') && Molpy.Boosts['DQ'].level > 2 && Molpy.CoalToAdd == 338372) Molpy.UnlockBoost('Chthonism');
	if(!Molpy.Got('Safety Canary') && Molpy.Boosts['DQ'].level > 2 && Molpy.DiamToSpend == 777777) Molpy.UnlockBoost('Safety Canary');
	...
	new Molpy.Boost({
		...
		desc: function(me){
			...
			if(me.bought && Molpy.groupBadgeCounts.diamm >= 30/* && Molpy.Has('Maps',)*/) str += '<br><br><input type="Button" onclick="Molpy.Coallate()" value="Dig"></input>' + 'out more space. This will make'
...
Molpy.Coallate = function(){
	...
	new Molpy.Boost({
		...
		icon: 'fort',
		desc: 'DRAGONS ONLY NO KNITES ALLOWED',
		draglvl: 'Wyvern',
		limit: function() { return (Molpy.Got('Marketing')?1:0) },
		price: {sand: 99995}
	})
	...
	new Molpy.Boost({
		...
		logic: 1e96,
		draglvl: 'Wyrm',
		stats: 'Think this boost name is dumb? It was nearly called Bananananasblitz!',
		lockFunction: function() {
			this.power = 0;
		},
		clickBeach: function() {
			if(Molpy.Boosts['Blitzing'].power) this.power++;
			Molpy.Add('Coal',this.power);
		},
	})
	new Molpy.Boost({
		name: '',
		icon: 'sblitz',
		className: 'alert',
		desc: 'There\'s coal in them thar newpix!',
		draglvl: 'Wyrm',
		price: {QQ: '72GW'}
	})

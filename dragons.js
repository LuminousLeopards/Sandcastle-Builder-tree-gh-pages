...
//					if(second) rwds.push('A' + (thing.single.match(/^[aeiou]/i)?'n ':' ') + thing.single);
					...
					num *= (n||1);
					if (Molpy.Got('Tuple or Nothing') && stuff == 'Gold') num*= Math.pow(2,loops);
...
Molpy.DragonDigRecalc = function() {
	...
	if (Molpy.Got('WotT')) Molpy.DragonDefenceMultiplier *= 10000;
...
Molpy.DragonFledge = function(clutch) {
	...
	if (npd && npd.amount > 0 ) {
		...
		if (npd.DragonType < dq.Level || (npd.DragonType == dq.Level && hatch.clutches[clutch] > npd.amount))	{ // Replace
			...
			oldDN = npd.amount;
			fight = 0;
			if(Molpy.Got('Honor Among Serpents')){
				dq.overallState = 0;
...
//	if (Molpy.CombatDebug) Molpy.Notify('Result = ' + result + ' factor ' + factor,1);
	...
	this.roboHatch = function(type, amount){
		if(Molpy.Has('Goats', Infinity) && Molpy.Level('Cryogenics') >= amount){
			...
			Molpy.Notify(' Fledged ' + amount + ' dragons from Cryogenics to replace the fallen.',1);
	...
	switch (result) {
		...
		case .5: //loop count exceeded and dragon health > localhealth
			Molpy.Notify(atktxt + '<br>Exhausted from the battle, both parties retreat to fight another day.',1);
			Molpy.Add('exp',Math.max(local.experience()*df.numb, Math.pow(10,npd.DragonType)/5));

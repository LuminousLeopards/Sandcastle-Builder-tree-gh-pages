...
Molpy.Up = function() {
	...
	Molpy.Wake = function() {
		...
		Molpy.ClickBeach = function(event, leopard, recursion) {
			...
			Molpy.Boosts['BananananaSblitz'].clickBeach();
			...
			this.getHTML = function(heading, level) {
				...
				if (Molpy.TotalDragons && Molpy.Boosts['DQ'].overallState == 0) { // Redundaknights
					...
					if(Molpy.NPdata[np].breath > 0 && Molpy.Boosts['DQ'].Level >=3 && !Molpy.Boosts['Dragon Breath'].breathRecovery){
						...
						//var effectswitch = Molpy.Breath(1,np);
//						str += breathtext;
						//if(Molpy.Boosts['DQ'].Level == 3) effectswitch = null;
//							str += effectswitch;
						str += breathtext;
...
//			}
		...
		Molpy.RewardRedacted = function(forceDepartment, automationLevel) {
			...
			} else if(Math.random() > .01 - Molpy.DragonLuck && dq.Level >= 2 && dq.overallState == 2 && !forceDepartment){
		...
		Molpy.RewardBlitzing = function() {			
			...
			if(Molpy.Got('Coald Rush') && Molpy.Redacted.totalClicks < 1e7) 
				//while(blitzTime) Molpy.Boosts['BananananaSblitz'].power = 1;
			if(Molpy.Got('Coald Rush') && Molpy.Redacted.totalClicks > 1e7){
				Molpy.Add('Coal',1);
	...
	Molpy.ONG = function() {
		...
		for(var i in Molpy.DragonStuffStats){
			i = 0;
		};
